...
进入 PRD、Issue 或实施计划前，先回答：

- 这个功能服务的最底层目标是什么。
- 如果去掉现有页面、代码和惯性做法，仍然必须存在的对象、状态和约束是什么。
- 当前方案是否只是因为已有实现方便，而不是因为它更接近最终业务闭环。
- 哪些能力应该先做最小可验证闭环，哪些只是后续放大器。

如果答案不能说明“为什么这是当前最该做的一步”，不得进入实现。
...
- 这个功能是否可能制造错误的完成感。
- 它是否绕过了权限、审计、版本、Workspace 隔离、可追溯或安全边界。
- 测试是否只证明了快乐路径，没有覆盖关键失败路径。
- 文档是否把第一切片误写成完整生产能力。
- 如果未来接入真实企业流程、真实通知或真实模型调用，当前抽象是否会阻塞演进。

发现问题时，优先修正范围、测试或文档；不能为了尽快提交而降低完成定义。
