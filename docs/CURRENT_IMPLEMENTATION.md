...
- 外部主动告警通知发送器。
- 预算审批、成本告警和成本治理详情页。
- 独立队列运营详情页、批量重投、释放租约、重投原因审计和取消原因审计。
...
当前已引入轻量 `execution_jobs` 队列骨架、失败重新入队、`dead_letter` 终态、worker 租约、heartbeat、观测页队列运营卡片、死信手动重新入队和主动取消，但仍未引入常驻后台 worker、真正的数据库行级并发锁、指数退避、独立队列运营详情页和外部通知 SDK。

## 17. 当前版本验证记录

已经完成：

- `apps/api/.venv/Scripts/python.exe -m pytest apps/api/tests -q`：后端 179 项测试通过。
...
3. 继续 V0.13 执行系统生产化：补常驻后台 worker、真正的数据库行级并发锁、指数退避、独立队列运营详情页、批量重投、取消/重投原因审计和实时事件推送，并评估 Temporal Signal/Update。
