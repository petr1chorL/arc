...
运行成功删除未补齐。HTTP Tool 独立异步测试已完成本地实现、双轴复审、22 程序与 7 场景浏览器；
后续空输出/同页幂等修复定向通过，70前端文件最终分片通过；期间卡死尚未确定根因，精确状态见 tool-test-verify.md。
后续提交 1940c36、2f51d8e 的 CI 均成功，Netlify 仍确认 b626602；未取得生产备份、恢复、最终对账、切流或观察证据。
...
> `docs/project-management/project-overview.md` 为准；历史段落不得覆盖该当前事实入口。
...
工作流连线已新增第一版字段映射契约：每条边可在 `edge.data.mappings` 中保存多组 `sourcePath -> targetPath`，前端连线配置面板可新增、编辑和删除映射行，保存草稿前会阻断字段不完整的映射。发布工作流版本时，连线映射随 `edges` 进入不可变快照。本阶段只保存和治理连线映射，不改变运行时变量传递。

工作流执行已接入第一版运行时连线映射骨架：当进入某个节点的入边配置了 `edge.data.mappings` 时，执行引擎会尝试把上游节点输出解析为 JSON，按 `$` 或 `$.a.b` 简单对象路径读取 `sourcePath`，写入 `targetPath`，并把生成的 JSON 文本作为下游节点输入。没有映射、上游输出不是 JSON 或路径缺失时，继续回退到原有“拼接上游输出，否则使用运行输入”的行为。
...
评估模板与运行链路已形成第一版闭环：Rubric 可声明 `judgeType=deterministic` 或 `judgeType=llm`；工作流可用的 LLM 模板版本必须包含唯一维度 ID、名称、权重、评分标准、绑定的 Workspace Model Provider 和模型标识。直接评估、Regression、Remediation 与工作流评估节点共用 `EvaluationService`；默认 `ModelJudgeGateway` 使用 OpenAI-compatible `ModelGateway` 请求逐维度分数、逐维度理由和总评理由，系统自行校验维度集合并计算加权分、总分与通过状态。Evaluation 记录保存不可变 Rubric 快照、`evaluatorType`、`evaluatorModel` 和 `evaluatorInput`；模型、Token、重试次数和成本会进入 NodeRun/Run 汇总。自动化测试使用 Fake Gateway，不依赖外部网络。
...
`npm run deploy:check`、`git diff --check` 与入口脚本 `sh -n` 通过。功能提交 `584bbec` 已通过
PR #19 合并；该修复继续包含在当前 GitHub `master` 与 Zeabur 共同部署的 `df1a0cd` 中。公网
`deployment.json` 返回该完整 SHA，`/healthz` 与 `/api/health` 均返回 200 和 `status=ok`。
因此生产启动已在版本与健康边界恢复。Docker Desktop 引擎未启动，根镜像本地运行证据仍缺失；
真实登录与 Seed completed/skipped 状态仍需保留独立人工签收证据。
...
V1.0 Lite 最短验收入口位于 `docs/V1_LITE_ACCEPTANCE_ENTRYPOINT.md`，完整自动验收脚本为 `scripts/verify-v1-lite.ps1`。该脚本会运行 V1 Lite 种子资产测试、V1 Lite 后端 E2E 测试、`npm run lint` 和 `npm run build`。

V1.0 Lite 真实服务验收证据采集脚本为 `scripts/accept-v1-lite.ps1`，底层调用 `scripts/v1-lite-acceptance.mjs`。该脚本要求通过环境变量或参数提供试点账号，不保存、不输出密码；运行成功后输出 Workspace、Reviewer、Workflow、Rubric、Run ID、Human Task ID、Evaluation ID、Regression Run ID、Trace ID 和 Notification Outbox 数量。该脚本会调用正在运行的 API 与真实 ModelGateway，因此 API 进程必须已配置 `MODEL_API_KEY` 或 Agent Provider `secretRef` 指向的环境变量；未配置时 Workflow 会在第一个 Agent 节点失败，不能作为 V1.0 Lite 签收证据。2026-06-29 已使用真实运行服务完成一次通过验收：Run `db29cdef-d074-4e05-96ae-bc017633482c`，Human Task `0d870943-8175-4058-adb9-8c73bd193585`，Evaluation `a9266557-718d-4ec9-b86e-7686128caee5`，Regression Run `aa2e880e-4443-423b-a02c-9a717372c046`，Trace `trace-db29cdef-d074-4e05-96ae-bc017633482c`。
...
Artifact 目录 API 还会返回轻量来源上下文，包括来源工作流名称、运行状态、来源节点名称、节点类型、节点状态、节点耗时和节点得分。Artifact 列表卡片展示工作流与节点名称；详情弹窗新增“来源上下文”区块，便于在产出物页面直接判断该 Artifact 的来源节点状态和质量结果。缺少历史 NodeRun 的 Artifact 仍可返回，来源节点字段为空并在前端回退到 ID 或“未知”。

Artifact 列表卡片现在也直接提供“查看运行链路”入口，链接会携带当前 Artifact 的 `runId` 与 `sourceNodeRunId`，不需要先打开详情弹窗即可进入对应 Observability 节点定位视图。详情弹窗中的同名入口继续保留。

运行中心现在是 Artifact 的主查看入口。工作流运行详情会按当前 Run 拉取 Artifact，并按节点运行逐项展示节点产出物；某个节点没有 Artifact 时明确显示“无”。侧栏不再把“产出物”作为独立主模块入口，历史 Artifact 路由仍保留用于深链和兼容入口。
...
| `/quality-operations` | `QualityOperations`，次级质量运营入口，不进入主导航 |
| `/runs` | `Runs` |
| `/reviews` | `Reviews` |
| `/observability` | `Observability`，兼容历史深链，不再作为主导航模块 |

`Layout` 作为共同外壳，负责侧栏、顶部栏和页面内容区域。

当前前端外壳不再展示通知运维、通知渠道、Data Object 和审计日志模块入口；相关页面路由也不作为当前 V1 Lite 工作台入口暴露，后续如需要再恢复。

工作流编排页当前也不再展示节点级 Data Object 契约配置；V1 Lite 先聚焦 Agent、人工审核和运行进度，结构化输入输出契约后续作为完整能力再恢复。
...
- 适用产出物。
- 评分维度的唯一 ID、名称、权重和评分标准。
- 硬性门禁。
- 自动通过分数。
- 评分器类型、Model Provider 和模型标识。
- 版本。
...
评估、人工审核、代码执行、等待节点和流程完成；历史 Gate 节点继续兼容。窄屏下节点库改为
横向滚动，仍可访问全部节点类型。
...
- 连线随草稿保存并在重新加载后恢复。
- 点击连线打开连线配置面板，查看上游节点、下游节点和连线 ID，并可单独删除当前连线。
- 新建工作流恢复 3 个默认节点和 2 条默认连线。
- 当前草稿与最近加载或保存版本不一致时显示“有未保存变更”，新建或切换工作流前需要确认是否放弃未保存变更，保存草稿成功后提示消失。
- 新增节点、复制节点、删除节点、删除连线和新增连线支持页面内撤销与重做；加载、新建、切换和保存成功后会重置历史栈。
- 工作流画布支持键盘快捷键：`Ctrl/Cmd+Z` 撤销，`Ctrl/Cmd+Y` 或 `Ctrl/Cmd+Shift+Z` 重做；选中连线后 `Delete` / `Backspace` 删除连线，选中节点后 `Delete` / `Backspace` 先确认再删除节点及关联连线。输入框、文本域、下拉框和可编辑内容中不会触发画布快捷键。
- 工作流级输入输出 Schema 编辑：编排页提供输入 Schema 与输出 Schema 文本域，保存草稿时随工作流一起提交；非法 JSON 或数组等非对象值会在前端阻断保存。
- 连线字段映射编辑：点击连线后可配置多组上游字段到下游字段的映射，映射保存在 `edge.data.mappings` 并随草稿和发布快照持久化。
- 小地图。
- 点击节点打开配置面板。
...
- 评估节点选择同 Workspace 已发布且可运行的 LLM Rubric Version；配置保存完整 `rubricRef`，不展示 Provider Secret Ref。
- 发布前校验评估节点恰好一条入边、模板版本存在且属于当前 Workspace，并校验 Human 节点分配、会签人数和 SLA 参数。
- Runtime 使用唯一上游成功 NodeRun 的产出物执行评估；成功时创建 Evaluation Record，并输出 `totalScore`、`passed`、`overallReason` 和全部 `dimensions`。失败时 Run 停止且不为失败节点创建 Artifact。
- 保存提示。
...
- 框选和分组节点。
- 多选和分组。
- 完整 JSONPath、数组路径、默认值、类型校验和表达式转换。
- 嵌套对象、数组、枚举、默认值和完整 JSON Schema 运行表单。
- 完整节点参数 Schema 编辑器。
- 循环、并行汇聚和子流程。
- 失败后的断点恢复。
- 并行节点、汇聚和条件路由执行。
...
文件：`src/pages/Evaluations.tsx`、`src/pages/QualityOperations.tsx`

该页面拆分已通过 PR #20 合并。2026-07-16 实时核对确认 GitHub `master`、Zeabur
`deployment.json` 均为 `df1a0cd`，`/api/health` 与 `/healthz` 均返回 `status=ok`。这证明
代码已经上线，不替代真实业务方对生产运行、审核、评分和回归闭环的独立签收。

路由职责：

- `/evaluations` 是主导航中的轻量评估模板库，只负责模板浏览与生命周期管理。
- `/quality-operations` 保留原有直接评估、Golden Set、Regression、趋势、失败分析、
  Remediation 和复测能力，作为不进入主导航的次级质量运营入口。
- 旧 `/evaluations?taskId=...` 与无 Workspace 前缀的历史深链会保留查询参数并重定向到
  `/quality-operations?taskId=...`；Artifact 创建修复任务后的入口也指向该次级页面。

### 8.1 评估模板库

- 页面初始化只读取 Workspace Rubric 与 Model Provider，不请求质量运营数据。
- 模板卡片展示状态、版本、维度数、通过分和绑定模型，不展示 Provider Secret Ref。
- 从 FastAPI 读取 Workspace 级 Rubric 卡片。
- 评分维度和权重。
- 硬性门禁。
- Rubric 支持 `judgeType=deterministic` 和 `judgeType=llm`。
- Rubric 支持记录 `judgeModel` 和 `modelProviderId`；LLM 模板发布快照冻结 Provider 引用与模型标识。
- 评分维度支持唯一 ID、名称、权重和评分标准。
- 当前 workspace 首次访问时会播种 3 个默认 Rubric，后续访问不会重复创建。
- 自动流转阈值。
- 新建 Rubric 草稿。
- 编辑 Rubric 名称、适用产出物、维度 ID/名称/权重/评分标准、硬性门禁和通过分数。
- 在 Rubric 配置弹窗中选择确定性评分器或 LLM Judge；LLM 模板选择可用 Model Provider 和模型标识。
- 发布不可变 Rubric 版本。
- 查看已发布 Rubric 版本快照。
- 停用 Rubric，停用后不允许继续编辑或发布。
- 前端校验必填字段、分数范围、维度 ID 唯一性、评分标准和维度权重合计；Provider 停用或配置不完整时不可用于新工作流评估节点。
...
- 展示最近反馈候选的状态、原因和标签。
- 无候选数据时展示空状态。
- 旧 Evaluation、Regression 与 Remediation 数据、数据库表和 API 均保留；页面搬迁不改变
  后端契约。

- 保存 Evaluation 记录，包含 Rubric 快照、每个维度的分数/权重/加权分/理由、系统计算总分、passed/failed 状态和总评理由。
- Evaluation 记录包含实际评分器类型、模型和可复现输入快照。
- `judgeType=llm` 的 Rubric 直接评估会通过可注入 Judge Gateway 执行。
- 默认 `ModelJudgeGateway` 会构造 Judge 输入快照、调用 OpenAI-compatible ModelGateway，并解析严格 JSON 评分结果。
- `ModelJudgeGateway` 会校验维度集合、0-100 分数和非空理由，并在 Judge 返回不可解析结果时有限重试；模型自报总分、权重和状态不会进入系统计算。
- 兼容旧模板的 Prompt 版本为 `llm-judge-v1`；逐维度解释模板使用 `llm-judge-explainable-v1`。
...
- 支持查看 Evaluation 记录详情，展示评估对象、运行时 Rubric 快照、维度权重、维度得分与理由、待评估产出物和总评理由。
...
- 历史确定性 Rubric 与现有质量运营能力继续兼容；工作流评估节点只允许选择已发布、维度可解释且 Provider 可用的 LLM Rubric Version。
- 已知残余风险：Rubric 发布版本号生成尚无数据库行级并发锁；同一 Rubric 的并发发布冲突仍需在后续生产化切片处理。
...
当前定位：该页面保留为历史深链和兼容入口，不再作为侧栏主导航模块。运行进度查看、节点产出物和流程状态筛选集中在运行中心。
...
本节原“下一步代码改造”早于 V0.16D-V0.31F，所列登录、队列、评估和 Notification
骨架已经部分或全部进入当前基线。2026-07-11 起不再从本节领取功能任务，当前顺序为：

1. 完成 P0 运行时安全人工签收和可能暴露模型 Key 的轮换。
2. 由真实业务方独立完成 V1.0 Lite 验收。
3. 根据试点证据选择可靠性治理或一个真实外部集成。

当前执行路线见 `docs/PROJECT_ROADMAP_TO_V1.md`；长期蓝图见
...
V0.23D 补齐工作流编排中心的单条连线编辑能力。用户点击 React Flow 连线后，右侧会打开连线配置面板，展示当前连线的上游节点、下游节点和连线 ID；选中连线时会清空当前选中节点，避免节点配置面板与连线配置面板同时出现。

点击“删除连线”只会从当前草稿图中移除该条边，不删除任何节点。删除后的边数组继续通过既有保存草稿请求体持久化，因此本版本不新增后端接口、不改变工作流草稿契约，也不实现拖拽重连、连线标签、条件表达式、多选连线、批量删除或撤销重做。验收文档见 `docs/ACCEPTANCE_V0.23D.md`。

## V0.23E 工作流未保存变更保护

V0.23E 在工作流编排中心补齐页面内未保存变更保护。页面会把当前草稿名称、节点和连线生成稳定签名，并与最近一次加载或保存成功后的基线比较；当两者不一致时，工具栏下方显示“有未保存变更”。签名计算会复用节点归一化逻辑，避免加载已有草稿时因为前端默认值补齐而误报未保存。

用户点击“新建”或从下拉框切换到其他工作流时，如果当前草稿存在未保存变更，页面会先弹出“放弃未保存变更？”确认框；选择“继续编辑”会保留当前画布和选中状态，选择“放弃变更并继续”才执行原本的新建或切换动作。保存草稿成功后会把服务端返回内容重新转换为画布图并刷新签名基线，因此未保存提示会消失。本版本不新增后端接口、不改变工作流草稿契约，也不实现自动保存、撤销重做、浏览器关闭/刷新拦截或跨路由离开保护。验收文档见 `docs/ACCEPTANCE_V0.23E.md`。

## V0.23F 工作流撤销与重做

V0.23F 在工作流编排中心补齐页面内撤销与重做能力。页面维护最多 50 步的画布历史快照，每个快照包含当前节点和连线；新增节点、复制节点、删除节点、删除连线和新增连线会先记录编辑前快照，再更新画布并清空重做栈。

点击“撤销”会恢复上一份画布快照，并把当前画布放入重做栈；点击“重做”会恢复刚撤销的画布。撤销或重做后会清空当前选中的节点和连线，避免 Inspector 指向已经不存在或过期的对象。加载工作流、新建工作流、切换工作流和保存草稿成功后会清空撤销/重做历史。本版本不新增后端接口、不改变工作流草稿契约，也不实现键盘快捷键、拖拽位置变化撤销、工作流名称撤销或跨路由持久化历史。验收文档见 `docs/ACCEPTANCE_V0.23F.md`。

## V0.23G 工作流编辑快捷键

V0.23G 在 V0.23F 的撤销/重做历史之上，为工作流编排中心增加页面级键盘快捷键。用户在画布或节点/连线选中状态下，可以使用 `Ctrl/Cmd+Z` 撤销画布编辑，使用 `Ctrl/Cmd+Y` 或 `Ctrl/Cmd+Shift+Z` 重做刚撤销的编辑。

选中连线后按 `Delete` 或 `Backspace` 会只删除当前连线并保留节点，且该操作进入撤销栈；选中节点后按 `Delete` 或 `Backspace` 会先弹出“删除选中节点？”确认框，提示将移除的关联连线数量，取消不改动画布，确认后删除节点及所有关联连线。本版本会跳过 `input`、`textarea`、`select` 和 `contenteditable`，避免用户编辑名称、Prompt、审核配置等表单时误触发画布快捷键。本版本不新增后端接口、不改变工作流草稿契约，也不实现快捷键自定义、命令面板、多选删除或拖拽位置变化撤销。验收文档见 `docs/ACCEPTANCE_V0.23G.md`。

## V0.24A 工作流输入输出 Schema

V0.24A 在工作流草稿上新增工作流级输入输出契约。后端 `WorkflowRecord` 新增 `input_schema` 和 `output_schema` JSON 字段，创建、更新和读取工作流时通过 `inputSchema` / `outputSchema` 暴露给前端；旧数据库会在启动迁移时补齐默认对象 Schema。

发布工作流版本时，`workflow_snapshot` 会把当时的输入 Schema 和输出 Schema 写入不可变快照，后续修改草稿不会改写已发布版本。前端工作流编排页新增“输入输出 Schema”区域，用户可以编辑两段 JSON 文本并随“保存草稿”提交；如果文本不是合法 JSON 对象，页面会显示错误并阻止调用保存接口。

本版本不改变工作流运行接口，不生成运行输入表单，也不实现节点级输入输出字段映射、Schema 可视化编辑器或 Data Object 资产。验收文档见 `docs/ACCEPTANCE_V0.24A.md`。

## V0.24B 连线字段映射

V0.24B 在工作流边契约上新增第一版字段映射配置。后端 `WorkflowEdge` 接收可选 `data` 对象，因此 `edge.data.mappings` 会随现有工作流草稿创建、更新、读取和发布链路保存；发布 WorkflowVersion 时，映射配置会和 `edges` 一起进入不可变快照。

前端 `WorkflowEdgeContract` 和 `src/domain/workflows.ts` 已支持 React Flow `Edge.data` 与 API `edge.data` 的双向转换。工作流编排页的连线配置面板新增“字段映射”区域，可以新增、编辑和删除映射行；保存草稿前会校验每条已存在映射必须同时填写上游字段和下游字段，否则显示错误并阻止保存。

本版本不改变工作流执行引擎，不根据映射自动传递节点输入输出，也不实现 Schema 字段选择器、JSONPath 校验、表达式转换或 Data Object 资产库。验收文档见 `docs/ACCEPTANCE_V0.24B.md`。

## V0.24C 运行时连线映射骨架

V0.24C 把 V0.24B 的连线字段映射接入工作流执行链路。`ExecutionService.execute_workflow_from` 在生成节点输入时，会优先检查该节点入边上的 `edge.data.mappings`：如果上游节点输出可解析为 JSON，就按简单路径读取 `sourcePath` 并写入目标对象的 `targetPath`，最后把目标对象序列化为 JSON 文本，作为下游节点 `input_text`。

当前路径语法只支持 `$` 和 `$.a.b` 形式的对象字段，不支持数组索引、JSONPath 过滤、表达式、默认值或类型转换。只有至少一条映射成功取到值时才使用映射结果；没有映射、源输出不是 JSON、路径无效或字段缺失时，执行引擎继续使用原有的上游输出拼接/运行输入回退行为，保证旧工作流兼容。

本版本不改变前端运行表单和运行 API 请求体，也不引入 Data Object 资产。验收文档见 `docs/ACCEPTANCE_V0.24C.md`。

## V0.24D Schema 运行输入表单

V0.24D 在工作流运行弹窗中接入第一版 Schema 输入体验。当前工作流的 `inputSchema` 如果是 `type: "object"`，且 `properties` 是一层对象字段，页面会生成结构化表单。已支持 `string`、`number`、`integer` 和 `boolean` 字段；字段标题优先使用 JSON Schema 的 `title`，否则使用字段名；`required` 数组中的字段会在开始运行前做必填校验。

提交时，前端会把表单值组装为 JSON 字符串，继续通过现有 `POST /workflows/{workflowId}/runs` 请求体中的 `input` 字段提交，因此后端运行 API 不变。`number` / `integer` 会转换为数字，`boolean` 使用 checkbox；空的非必填字段不会写入 JSON。

空 Schema、没有 `properties` 或字段类型超出当前支持范围时，运行弹窗继续显示原来的“运行输入”自由文本框。本版本不实现嵌套对象、数组、枚举、默认值、格式校验、完整 JSON Schema 渲染器或后端 Schema 校验。验收文档见 `docs/ACCEPTANCE_V0.24D.md`。
## V0.24E Schema 字段选择器

工作流连线映射面板已经接入第一版 Schema 字段快捷选择能力：当当前工作流声明了简单对象型 `inputSchema.properties` 时，连线映射行会显示源字段和目标字段下拉选择。源字段选择会写入 `$.<field>`，目标字段选择会写入 `$.input.<field>`，并继续通过现有 `edge.data.mappings` 保存到工作流草稿。

这一版只做一层字段快捷选择，不改变后端 API、运行时映射引擎或连线数据契约。自由文本路径输入仍然保留，用于高级路径或暂未被 Schema 覆盖的场景。嵌套 Schema 字段树、完整 JSONPath Builder、节点输出 Schema 与 Data Object 目录仍属于后续版本。

验收记录见 `docs/ACCEPTANCE_V0.24E.md`。

---

## V0.24F 运行输入 Schema 后端校验

工作流运行入口现在会在创建 Run 或入队异步执行前读取已发布 WorkflowVersion 快照中的 `inputSchema`，并对当前请求的 `input` 做第一版后端轻量校验。当前支持简单一层对象字段：`string`、`number`、`integer` 和 `boolean`；`required` 中指向这些支持字段的条目会被当作必填字段校验。

当 Schema 可校验时，运行输入必须是合法 JSON 对象；缺少必填字段或字段类型不匹配会返回 `422`，并且不会创建 `WorkflowRunRecord`、不会创建异步执行 Job、不会调用模型。合法输入继续沿用现有 `input` 字符串契约进入工作流执行。

空 Schema、没有可支持字段或字段类型超出当前轻量校验范围时，后端保持旧行为，继续允许自由文本输入。本版本不实现完整 JSON Schema 引擎，不支持嵌套对象、数组、枚举、格式、正则、默认值或 Data Object 资产。验收记录见 `docs/ACCEPTANCE_V0.24F.md`。

---

## V0.25A Data Object Definition 后端骨架

平台新增 Workspace 级 Data Object Definition 后端资产。后端新增 `data_object_definitions` 和 `data_object_versions` 两张表，Definition 保存名称、描述、JSON Schema、状态、最新版本、创建人和时间；Version 保存 Definition ID、版本号和不可变快照。新表已加入迁移 Workspace 表清单。

API 新增 `GET /api/workspaces/{workspaceId}/data-objects`、`POST /api/workspaces/{workspaceId}/data-objects` 和 `POST /api/workspaces/{workspaceId}/data-objects/{definitionId}/publish`。创建时同一 Workspace 内名称唯一；不同 Workspace 隔离。发布使用 `next_version` 生成 `v1.0.0`、`v1.1.0` 等版本号，并把当时的 Definition 内容冻结到 `snapshot`，后续修改主记录不会改写旧版本快照。
...
## V0.26A 最终 ArtifactVersion 记录 Data Object 契约

工作流运行完成时，最终产出物现在会同时创建 `ArtifactRecord` 和 `ArtifactVersionRecord`。如果当前 WorkflowVersion 快照中的产出链路节点带有已冻结的 `outputDataObjectRef`，最终 ArtifactVersion 会记录对应的 `dataObjectDefinitionId`、`dataObjectVersionId` 和 `dataObjectSnapshot`。

这让一次工作流运行的具体 Artifact 能追溯到当时绑定的 Data Object Version，而不是只保存一段文本。后续 Data Object Definition 编辑或重新发布，不会改变旧 ArtifactVersion 上保存的契约快照。

未绑定输出 Data Object 的旧工作流仍会生成普通 ArtifactVersion，相关 Data Object 字段为空。Agent 直接测试运行不读取 WorkflowVersion 快照，继续保持原有运行路径。本版本不校验 Artifact 内容是否符合 Schema，不提供 Artifact 实例列表，不处理 Human Task 修改版本的契约继承，也不提供 Data Object 影响面查询。验收记录见 `docs/ACCEPTANCE_V0.26A.md`。

---

## V0.26B Artifact 目录查询 API

平台新增 Workspace 级 Artifact 目录查询接口：`GET /api/workspaces/{workspaceId}/artifacts`。接口返回 ArtifactVersion 视角的产物实例列表，包含 `artifactId`、`artifactVersionId`、`runId`、`sourceNodeRunId`、`content`、`score`、创建时间以及可为空的 Data Object Definition ID、Data Object Version ID 和 Data Object Snapshot。

接口支持 `dataObjectDefinitionId` 查询参数过滤，便于后续从某个 Data Object Definition 反查具体产物实例。查询按当前 Workspace 隔离，并沿用 `run.read` 权限保护。本版本不提供前端 Artifact 实例页，不支持分页游标、全文搜索、排序配置或导出，也不返回完整 Run / NodeRun 详情。验收记录见 `docs/ACCEPTANCE_V0.26B.md`。

---

## V0.26C Artifact 实例页

前端新增 Workspace 内 Artifact 实例页：`/w/:workspaceSlug/artifacts`，主导航新增“产出物”入口。页面通过 `GET /api/workspaces/{workspaceId}/artifacts` 读取 ArtifactVersion 列表，展示当前列表数量、Data Object 绑定数量、平均得分、Run ID、来源 NodeRun ID、Data Object Definition ID、Data Object Version ID、Schema 摘要和内容预览。

页面支持输入 Data Object Definition ID 后筛选列表，便于从某个数据对象定义追踪具体产物实例。旧的 Data Object Definition 页面仍负责定义资产管理；Artifact 实例页只做运行产物查看。本版本不提供 Artifact 详情页，不做完整 JSON 结构化渲染或 Schema 校验，不支持分页游标、全文搜索、导出，也不从 Data Object Definition 列表生成下拉筛选。验收记录见 `docs/ACCEPTANCE_V0.26C.md`。

---

## V0.26D Artifact 详情弹窗

Artifact 实例页新增只读详情弹窗。用户可以在任意 Artifact 卡片上点击“查看详情”，查看 ArtifactVersion ID、Artifact ID、Run ID、来源 NodeRun ID、Data Object Version ID、Score、格式化后的 Artifact 内容以及 Data Object Snapshot。

详情弹窗直接使用当前列表数据，不新增后端接口，也不引入独立详情 URL。Artifact 内容如果是合法 JSON，会格式化展示；无法解析时保留原文。Data Object Snapshot 以格式化 JSON 展示，便于检查当时冻结的结构契约。本版本不做复制、下载、导出、编辑或 Schema 校验。验收记录见 `docs/ACCEPTANCE_V0.26D.md`。

---

## V0.26E Artifact Schema 轻量校验状态

Artifact 实例页新增前端轻量 Schema 校验状态。页面会基于 Artifact 内容和当前记录携带的 Data Object Snapshot 派生“Schema 校验通过”“Schema 校验失败”或“未校验”，并在列表卡片中展示紧凑状态标签。
...
## V0.27B Artifact 筛选 URL 同步

Artifact 实例页新增筛选条件 URL 同步能力。用户访问 `/w/:workspaceSlug/artifacts?dataObjectDefinitionId=<id>&schemaValidationStatus=<status>` 时，页面会用 query 参数初始化 Data Object Definition ID 与 Schema 校验状态筛选控件，并用同一组参数请求 Artifact 目录 API。
...
## V0.27D Artifact 节点级追溯锚点

Artifact 详情弹窗中的“查看运行链路”入口现在会同时携带 `runId` 和 `nodeRunId`。链接格式为 `/w/:workspaceSlug/observability?runId=<runId>&nodeRunId=<sourceNodeRunId>`，其中两个参数都会进行 URL 编码。
...
## V0.27F Observability 反向查看产出物

Artifact 目录 API 新增 `runId` 和 `sourceNodeRunId` 查询参数，可定位某次 Workflow Run、某个 NodeRun 生成的 ArtifactVersion。前端 Artifact 页面会从 URL 初始化这两个筛选，并在页面顶部展示当前 Run / NodeRun 筛选条件。

Observability 运行详情中的每个节点卡片新增“查看产出物”链接，指向 `/w/:workspaceSlug/artifacts?runId=<runId>&sourceNodeRunId=<nodeRunId>`。这让用户可以从产出物详情进入运行链路，也可以从运行链路节点反向回到对应产出物列表。

本版本不新增 Artifact 专属详情 API；如果某个 NodeRun 没有产出物，跳转后显示空列表。验收记录见 `docs/ACCEPTANCE_V0.27F.md`。

---

## V0.27G Artifact 来源上下文

Artifact 目录 API 现在会返回轻量来源上下文：`workflowName`、`runStatus`、`sourceNodeName`、`sourceNodeType`、`sourceNodeStatus`、`sourceNodeDurationMs` 和 `sourceNodeScore`。这些字段来自现有 WorkflowRun 与 NodeRun 记录，并继续受 Workspace 隔离保护。

Artifact 实例页会在卡片中展示来源工作流和节点名称，并在详情弹窗中新增“来源上下文”区块，展示运行状态、节点类型、节点状态、节点耗时和节点得分。旧 Artifact 如果找不到来源 NodeRun，API 仍返回记录，节点字段为 `null`，前端回退到 NodeRun ID 或“未知”。

本版本不新增 Artifact 独立详情接口，不提供来源上下文筛选、排序、搜索或导出。验收记录见 `docs/ACCEPTANCE_V0.27G.md`。

---

## V0.27H Artifact 卡片直达运行链路

Artifact 列表卡片新增“查看运行链路”入口。该链接复用详情弹窗中的追溯路径，指向 `/w/:workspaceSlug/observability?runId=<runId>&nodeRunId=<sourceNodeRunId>`，并在可访问名称中包含 ArtifactVersion ID，便于测试和读屏器识别。

这让排障人员在列表中看到失败 Artifact 后，可以直接进入对应运行链路，不必先打开 Artifact 详情。原有“查看详情”和详情弹窗中的“查看运行链路”入口保持不变。

本版本不新增后端 API，不改变 Observability 页面参数语义，也不新增批量跳转或新窗口行为。验收记录见 `docs/ACCEPTANCE_V0.27H.md`。

---

## V0.28A Artifact 创建修复任务入口

Artifact 列表卡片现在可以把失败或低分产出物直接接入现有 Remediation Task 闭环。当前 Artifact 的 Schema 校验状态为 `failed`，或评分低于 75 分时，卡片会显示“创建修复任务”按钮；点击后前端复用现有 `POST /api/workspaces/{workspaceId}/evaluations/remediation-tasks` 接口创建任务。
...
评估中心的 Remediation Task 卡片新增来源反查入口。对于 `clusterKey=artifact:<artifactVersionId>` 的任务，卡片会显示“查看产出物”链接，指向当前 Workspace 的 `/artifacts?artifactVersionId=<artifactVersionId>`，复用 Artifact 详情深链能力自动打开对应产出物详情。

所有带 `sourceRunId` 的任务卡片都会显示“查看运行链路”链接，指向 `/observability?runId=<sourceRunId>`。这让用户从修复任务继续处理时，可以直接回到来源 Workflow Run 的 Trace 与执行事件，不需要复制 ID 后手动检索。

链接的可访问名称包含 Remediation Task ID，便于自动化测试和读屏器识别。非 Artifact 聚类任务不会显示“查看产出物”入口。本版本不新增单条 Remediation Task 详情页，不新增后端接口，也不从任务建议动作文本中解析 NodeRun ID。验收记录见 `docs/ACCEPTANCE_V0.28C.md`。

---

## V0.28D 修复任务详情视图

评估中心现在会基于 `taskId` query 展示聚焦的 Remediation Task 详情视图。用户访问 `/w/:workspaceSlug/evaluations?taskId=<remediationTaskId>` 且任务存在时，页面在 Remediation Tasks 看板上方展示“修复任务详情”区域，包含任务标题、优先级、状态、负责人、截止时间、来源 Run、聚类 Key、样本数量、建议动作和处理时间线。

详情视图复用 V0.28C 的来源反查入口：Artifact 来源任务可直接“查看产出物”，所有任务都可“查看运行链路”。Remediation Task 卡片新增“打开详情”链接，指向当前 Workspace 的 `/evaluations?taskId=<taskId>`，便于从任务列表进入可分享的详情上下文。

本版本不新增独立任务详情路由，不新增后端单任务读取接口，也不把评论表单和状态流转按钮移出原任务卡片。验收记录见 `docs/ACCEPTANCE_V0.28D.md`。

---

## V0.28E 修复任务详情处理动作

评估中心的“修复任务详情”区域现在不仅能查看任务，也能直接处理任务。详情区新增“标记处理中”“标记完成”和条件展示的“发起复测”按钮，复用现有 Remediation Task 状态流转和复测 API；任务卡片中的同名动作继续保留。

详情区新增“详情评论内容”和“详情附件引用”表单，提交后会复用现有评论接口写入处理活动。详情时间线和任务卡片时间线都从同一份任务状态渲染，因此评论、附件、状态和复测结果会保持同步。

本版本不新增后端接口，不新增独立任务详情页，不改变 Remediation Task 去重、权限、状态机或复测回流规则。验收记录见 `docs/ACCEPTANCE_V0.28E.md`。

---

## V0.28F 修复任务详情运营字段编辑

评估中心的“修复任务详情”区域现在可以直接编辑 Remediation Task 的运营字段。详情区新增 `详情负责人`、`详情优先级`、`详情截止日期` 和“保存任务信息”，处理人可以在同一个详情上下文中修正负责人、优先级和截止时间。

后端复用既有 `PATCH /api/workspaces/{workspaceId}/evaluations/remediation-tasks/{taskId}`，请求体现在支持可选 `status`、`owner`、`priority` 和 `dueDate` 字段，并要求至少包含一个更新字段。`owner` 的空字符串或 `null` 会归一化为未分配，`priority` 仅允许 `P0`、`P1` 和 `P2`，`dueDate` 支持 ISO datetime 或 `null`。运营字段发生变化时，系统会写入一条 `metadata_change` 处理记录，记录负责人、优先级和截止时间的前后变化。

前端详情区保存成功后继续通过同一份 Remediation Task 状态刷新详情区和任务卡片，因此负责人筛选、优先级标签、截止日期展示和处理时间线会同步更新。原有评论、附件、状态流转、复测和来源反查入口保持不变。本版本不新增成员选择器、批量转派、字段级权限差异或独立任务详情 API。验收记录见 `docs/ACCEPTANCE_V0.28F.md`。

---

## V0.29A 修复任务详情复测风险摘要

Remediation Task API 响应现在包含后端派生的 `retestSummary` 字段，用统一口径说明任务当前复测风险状态。未发起复测时返回 `not_run / 未复测`，并提示先标记完成后发起复测；复测后如果失败样本数大于 0，返回 `failed / 复测未通过`，同时包含失败样本数、通过率和继续修复建议；复测全部通过时返回 `passed / 复测通过`。
...
评估中心的“修复任务详情”区域新增“复制任务链接”按钮。点击后前端会基于当前浏览器 origin 和当前 Workspace 路径生成标准任务详情 URL：`/w/:workspaceSlug/evaluations?taskId=<taskId>`，并调用 `navigator.clipboard.writeText` 写入剪贴板。

复制成功后详情区展示“已复制修复任务链接”；如果浏览器拒绝剪贴板权限或写入失败，详情区展示“复制失败，请手动复制地址栏链接”。切换定位任务时，上一条任务的复制提示会被清空，避免把旧反馈带到新的任务上下文。

本版本不新增后端接口、数据库字段、短链服务、分享权限模型或审计事件；它只补齐 V0.29B 深链直读后的协作入口。验收记录见 `docs/ACCEPTANCE_V0.29C.md`。

---

## V0.29D 修复任务详情关闭入口

评估中心的“修复任务详情”区域新增“关闭详情”按钮。用户从任务列表或分享链接进入 `/w/:workspaceSlug/evaluations?taskId=<taskId>` 后，可以点击该按钮退出聚焦任务上下文；前端会从当前 URL query 中移除 `taskId`，并让详情区域隐藏，Remediation Tasks 列表继续保留。

关闭详情只修改前端路由状态，不重新定义 Remediation Task 状态机，也不清空任务筛选、评论草稿或已加载任务列表。后续如果同一页面存在其他 query 参数，该动作只负责移除 `taskId`。

本版本不新增后端接口、数据库字段、独立详情路由、审计事件或浏览器历史自定义逻辑。验收记录见 `docs/ACCEPTANCE_V0.29D.md`。

---

## V0.29E 修复任务无效深链恢复

评估中心现在会在无效 Remediation Task 深链提示中提供“清除定位”按钮。当 URL 中的 `taskId` 无法通过当前列表或单条详情接口解析时，页面仍会展示 Remediation Tasks 列表，并在错误提示旁提供一键恢复入口。
...
## V0.30B Notification Outbox Worker 入口

平台新增 `app.notification_worker` 模块，提供 `NotificationOutboxWorker`、`create_notification_outbox_worker` 和命令行入口。Worker 会读取所有 active Workspace，复用 V0.30A 的 `NotificationOutboxDispatchService` 消费 pending 通知并提交状态回写。它支持单轮处理、处理到空队列和常驻轮询三种模式。
...
## V0.30G Notification Ops View

前端新增通知运维页面 `/w/:workspaceSlug/notifications`，接入 V0.30F 的 Workspace 级 Notification Outbox 查询 API。侧边栏新增“通知运维”入口，legacy `/notifications` 会重定向到当前 Workspace 路径。

页面支持按状态、渠道和失败码筛选通知，并展示当前筛选结果的总数、失败数、待发送数和已发送数。通知列表展示事件类型、接收人、状态、渠道、失败码、错误文本和创建时间，帮助运维人员先定位事实，再决定是否进入后续恢复动作。

本版本只读，不新增发送、批量重新入队、真实外部通知渠道、告警发送、单条通知详情页或新的后端接口。验收记录见 `docs/ACCEPTANCE_V0.30G.md`。

---

## V0.30H Notification Requeue View

通知运维页面现在可以对 `status=failed` 的 Notification Outbox 记录发起手动重新入队。页面只在失败通知卡片上展示“重新入队”入口；点击后要求填写重新入队原因，空原因会在前端阻断提交。

前端新增 `requeueNotification(workspaceId, notificationId, reason)`，复用既有 `POST /api/workspaces/{workspace_id}/notifications/outbox/{notification_id}/requeue` 后端接口，并以 JSON 请求体提交 `{ reason }`。提交成功后页面会刷新当前筛选下的 Notification Outbox 列表；提交失败时保留原因输入区并展示错误，避免用户误以为恢复已经成功。

本版本不新增批量重投、自动重试、真实外部发送、通知详情页、权限差异 UI 或新的后端接口。验收记录见 `docs/ACCEPTANCE_V0.30H.md`。

---

## V0.30I Notification Dispatch View

通知运维页面新增“触发发送器”入口，复用既有 `POST /api/workspaces/{workspace_id}/notifications/outbox/dispatch` 后端接口，对当前 Workspace 执行一次 Notification Outbox dispatch pass。前端新增 `NotificationDispatchSummary` / `NotificationDispatchItem` 类型和 `dispatchNotifications(workspaceId)` API 封装。

触发成功后，页面展示最近一次发送器摘要，包括本次处理数量、已发送数量和失败数量，并刷新当前筛选条件下的 Notification Outbox 列表。触发失败时，页面展示 API 错误且不展示新的成功摘要，避免用户误判发送器已经消费。

本版本不新增真实外部渠道、单条通知发送、批量选择、自动轮询、后台 Worker 控制台、后端接口或数据库字段。验收记录见 `docs/ACCEPTANCE_V0.30I.md`。

---

## V0.30J Notification Dispatch Details

通知运维页的发送器结果区新增“本次明细”，复用 V0.30I dispatch API 返回的 `items`，展示每条通知的 id、eventKey、status、channel、errorCode、providerMessageId 和 error。失败项会保留稳定失败码与错误文本，便于从“失败了几条”进一步定位到“哪条失败、在哪个渠道失败、失败码是什么”。当本次 dispatch 返回空 `items` 时，页面展示“本次没有返回明细”。

从第一性原理看，本版本只解决一个最小问题：发送器汇总数无法支撑排障，必须把本次返回的失败事实暴露给操作员。对抗式审查后的边界是：该区域不是发送历史，不新增持久化查询、单条详情页、单条重试、自动重试、新后端接口或真实外部渠道。验收记录见 `docs/ACCEPTANCE_V0.30J.md`。

---

## V0.31A Notification Channel Assets

平台新增 Workspace 级通知渠道资产骨架：`notification_channels` 保存渠道名称、渠道类型、状态、非密钥配置、`secretRef` 标签、创建人与时间戳。后端新增 `POST /api/workspaces/{workspace_id}/notification-channels`、`GET /api/workspaces/{workspace_id}/notification-channels` 和 `POST /api/workspaces/{workspace_id}/notification-channels/{channel_id}/disable`。

创建渠道时，`config` 必须是 JSON object；同一 Workspace 内渠道名称重复会返回 409；列表只返回当前 Workspace 渠道并按创建时间倒序；停用渠道会把状态改为 `disabled`、更新 `updatedAt` 并写入 `notification_channel.disable` 审计事件。跨 Workspace 停用或不存在渠道返回 404。

从第一性原理看，本版本解决真实外部通知前的治理边界：平台必须先知道当前 Workspace 允许哪些渠道存在、是否启用，以及其非密钥配置和凭证引用标签。对抗式审查后的边界是：本版本不接入真实飞书、邮件、Webhook 发送，不解析或保存密钥值，不做连接测试、通知模板、限流、退避、幂等或前端渠道配置页面。验收记录见 `docs/ACCEPTANCE_V0.31A.md`。

---

## V0.31B Notification Channel Settings

前端新增通知渠道配置页 `/w/:workspaceSlug/settings/notification-channels`，侧栏设置区新增“通知渠道”入口，页面接入 V0.31A 的 Workspace 级通知渠道 API。页面可以加载当前 Workspace 的渠道资产，展示名称、类型、状态、`secretRef` 标签、非密钥配置和更新时间。

页面新增创建表单，包含渠道名称、渠道类型、`secretRef` 标签和配置 JSON。前端会在提交前校验配置必须是合法 JSON object，创建成功后把新渠道插入列表顶部并清空表单。active 渠道卡片提供“停用”动作，停用成功后状态更新为 `disabled`，并移除该卡片上的停用入口。

从第一性原理看，本版本让 V0.31A 的后端渠道治理边界变成 Workspace 管理员可操作的界面闭环。对抗式审查后的边界是：页面不填写或展示真实密钥值，不做连接测试，不触发真实外部发送，不新增通知模板、限流、退避或幂等能力。验收记录见 `docs/ACCEPTANCE_V0.31B.md`。

---

## V0.31C Notification Channel Dispatch Guard

默认 Notification Outbox dispatch 现在会在发送前检查 Workspace 通知渠道资产状态。`in_app` 继续作为内置默认渠道，不要求资产记录；非 `in_app` 渠道如果没有同类型渠道资产，会失败并返回稳定错误码 `notification_channel_missing`；如果同类型渠道资产存在但全部处于 `disabled`，会失败并返回 `notification_channel_disabled`。

当 Workspace 至少有一个 active 同类型渠道资产时，dispatch 会继续交给既有代码 dispatcher/router。如果当前后端还没有注册真实 adapter，仍会返回既有 `channel_not_configured`，用于区分“治理资产已配置”与“代码 adapter 尚未接入”。显式注入自定义 dispatcher/router 的测试和扩展路径不会启用该资产预检，避免破坏 adapter 单元测试边界。

从第一性原理看，本版本让发送失败原因能够解释治理状态，而不是把资产缺失、资产停用和 adapter 缺失混成一个失败。对抗式审查后的边界是：active 渠道资产不等于真实外发能力，本版本不新增真实飞书、邮件、Webhook adapter，不解析密钥，不新增数据库字段、连接测试或前端页面。验收记录见 `docs/ACCEPTANCE_V0.31C.md`。

---

## V0.31D Notification Remediation Guidance

通知运维页现在会把失败码翻译成排障建议，并在通知列表和发送器结果明细的失败项中展示。`notification_channel_missing` 会提示当前 Workspace 缺少对应通知渠道资产；`notification_channel_disabled` 会提示渠道资产已停用；`channel_not_configured` 会提示治理资产可能已配置但后端 adapter 尚未接入；`channel_disabled` 和 `provider_timeout` 也会给出对应处理方向。

建议区保留原始失败码和错误文本，同时提供“打开通知渠道”链接，指向当前 Workspace 的 `/settings/notification-channels`。失败码筛选项也新增 `notification_channel_missing` 和 `notification_channel_disabled`，便于快速定位渠道治理类失败。

从第一性原理看，本版本让失败证据进入可执行下一步，而不是要求运维人员理解后端错误码。对抗式审查后的边界是：建议只是说明下一步，不会自动创建渠道、启用渠道、重新入队、连接真实外部通知或隐藏原始失败证据。验收记录见 `docs/ACCEPTANCE_V0.31D.md`。

---

## V0.31E Notification Channel Impact

通知渠道设置页现在会额外读取当前 Workspace 的失败 Notification Outbox，并按渠道类型聚合失败影响面。每张渠道资产卡片展示该渠道当前关联的失败通知数量；有失败通知时展示主要失败码，例如 `notification_channel_missing` 或 `channel_not_configured`。

渠道卡片新增“查看通知运维”链接，指向当前 Workspace 的 `/notifications?channel=<channelType>`，让管理员能从配置页回到通知运维页继续排障。失败影响面加载失败时，页面会展示“失败影响面加载失败”，但不会阻断渠道资产列表、创建渠道或停用渠道。

从第一性原理看，本版本让渠道治理页回答“这个渠道当前影响多少失败通知”，承接 V0.31D 的排障建议闭环。对抗式审查后的边界是：影响面只是运营提示，不代表真实外部投递、自动恢复、连接测试或后端聚合能力；页面仍不展示密钥值。验收记录见 `docs/ACCEPTANCE_V0.31E.md`。

---

## V0.31F Notification Channel Enable

通知渠道资产新增恢复启用能力。后端新增 `POST /api/workspaces/{workspace_id}/notification-channels/{channel_id}/enable`，把当前 Workspace 内的渠道状态改为 `active` 并更新 `updatedAt`；跨 Workspace 或不存在渠道返回 404。恢复操作会写入 `notification_channel.enable` 审计事件，记录 before/after status、渠道名称和渠道类型。

通知渠道设置页现在会对 `disabled` 渠道展示“恢复启用”按钮；恢复成功后卡片状态即时变为 `active`，并重新显示“停用”入口。失败时页面展示错误，不提前修改状态。

从第一性原理看，本版本补齐渠道治理状态机的人工恢复闭环，让 `notification_channel_disabled` 的排障建议可执行。对抗式审查后的边界是：恢复 active 只代表平台治理状态，不代表真实外部渠道连通，不做连接测试，不自动重新入队失败通知，也不展示或解析密钥值。验收记录见 `docs/ACCEPTANCE_V0.31F.md`。
