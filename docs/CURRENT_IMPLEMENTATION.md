...
工作流执行新增异步队列第一切片：`RunCreate.asyncMode=true` 时会创建状态为 `排队中` 的 Run 与 `execution_jobs` 队列任务，不立即调用模型；`POST /execution-jobs/next` 可领取当前 Workspace 下一条 `queued` job 并执行，执行完成后回写 Run、NodeRun 和 job 状态。失败但未达到 `max_attempts` 的 job 会重新进入 `queued`，Run 回到 `排队中` 并保留错误原因；达到最大尝试次数后 job 进入 `dead_letter`。默认 `asyncMode=false` 的同步执行路径保持不变。
...
当前已引入轻量 `execution_jobs` 队列骨架、失败重新入队和 `dead_letter` 终态，但仍未引入常驻后台 worker、租约/心跳、指数退避、死信队列运营页和外部通知 SDK。

## 17. 当前版本验证记录

已经完成：

- `apps/api/.venv/Scripts/python.exe -m pytest apps/api/tests -q`：后端 174 项测试通过。
...
3. 继续 V0.13 执行系统生产化：补常驻后台 worker、租约/心跳、指数退避、死信队列运营页、主动终止和实时事件推送，并评估 Temporal Signal/Update。
