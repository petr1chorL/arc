...
当前已引入轻量 `execution_jobs` 队列骨架、失败指数退避重新入队、`dead_letter` 终态、worker 租约、heartbeat、观测页队列运营卡片、死信手动重新入队、主动取消、常驻 worker 代码骨架、worker CLI 入口和 Compose worker 服务定义，但仍未引入操作系统级 worker 服务、真正的数据库行级并发锁、独立队列运营详情页和外部通知 SDK。
