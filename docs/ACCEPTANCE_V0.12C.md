...
- 默认 HTTP Gateway 支持 GET / POST、host allowlist、超时配置和响应摘要。
- 未配置 allowlist 或 host 不在 allowlist 时，HTTP Gateway 会拒绝调用，不会发起外联。
- MCP Tool 支持可注入网关的测试调用，并会写入调用日志。
...
- 6 项通过。
- 7 项通过。
- 覆盖成功调用日志、失败脱敏日志、非 HTTP / MCP Tool 422、MCP 测试调用日志、Agent 运行写入带上下文的工具调用日志、HTTP allowlist 拦截和 MockTransport 成功调用。
...
```powershell
apps/api/.venv/Scripts/python.exe -m pytest apps/api/tests/test_tool_runtime_api.py apps/api/tests/test_execution_api.py apps/api/tests/test_observability_api.py apps/api/tests/test_model_gateway.py apps/api/tests/test_security.py -q
```

预期结果：

- 36 项通过。
- 覆盖工具 Runtime、执行链路、观测链路、网关配置和安全设置回归。

```powershell
apps/api/.venv/Scripts/python.exe -m pytest apps/api/tests/test_tool_runtime_api.py apps/api/tests/test_execution_api.py apps/api/tests/test_observability_api.py apps/api/tests/test_tool_skill_assets_api.py apps/api/tests/test_tool_skill_invocation_logs_api.py -q
```

预期结果：

- 32 项通过。
- 覆盖 MCP 测试调用、HTTP / Agent 执行、观测事件、资产库和调用日志回归。
