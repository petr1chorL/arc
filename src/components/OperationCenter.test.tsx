...
describe('durable operation recovery', () => {
  ...
  it('restores a Tool operation across remount, hides raw results and derives asset control permission', async () => {
    const fetchMock = vi.fn().mockImplementation(() => Promise.resolve(new Response(JSON.stringify({ ...completed, kind: 'tool.test', status: 'queued', result: null }))))
    vi.stubGlobal('fetch', fetchMock)
    sessionStorage.setItem('arc-operations:user-1:ws-1', '["op-1"]')
    const view = render(<WorkspaceProvider workspace={{ ...workspace, role: 'operator' }}><OperationCenter /></WorkspaceProvider>)
    await screen.findByText('已接收，排队中')
    expect(screen.queryByRole('button', { name: '取消后续执行' })).not.toBeInTheDocument()
    view.rerender(<WorkspaceProvider workspace={{ ...workspace, role: 'builder' }}><OperationCenter /></WorkspaceProvider>)
    expect(screen.getByRole('button', { name: '取消后续执行' })).toBeInTheDocument()
    view.unmount()
    fetchMock.mockImplementation(() => Promise.resolve(new Response(JSON.stringify({ ...completed, kind: 'tool.test', result: { invocationId: 'op-1', status: 'succeeded', output: 'PRIVATE_TOOL_OUTPUT' } }))))
    render(<WorkspaceProvider workspace={workspace}><OperationCenter /></WorkspaceProvider>)
    await screen.findByText('已完成')
    expect(screen.queryByText(/PRIVATE_TOOL_OUTPUT/)).not.toBeInTheDocument()
    expect(screen.queryByText('查看持久化结果')).not.toBeInTheDocument()
    expect(fetchMock).toHaveBeenCalledTimes(2)
  })
})
