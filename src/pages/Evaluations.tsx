...
export function Evaluations() {
  ...
  const loadOverview = useCallback(async () => {
    ...
    setError('')
    try {
      setOverview(await getEvaluationOverview(workspace.id))
    } catch (loadError) {
      setError(loadError instanceof Error ? loadError.message : '评估资产概览加载失败')
    } finally {
      setIsLoading(false)
    }
  }, [workspace.id])
    ...
    <div className="page-stack">
      <section className="page-toolbar">
        ...
        <div className="toolbar-actions">
          <button className="button secondary" type="button" onClick={() => void loadOverview()} disabled={isLoading}>
