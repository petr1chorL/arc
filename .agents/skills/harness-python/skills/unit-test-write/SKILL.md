---
name: unit-test-write-python
stage: ③ 单元测试编写
description: 为实现代码编写 Python 单元测试，核心逻辑覆盖率 ≥80%，覆盖 AC 与边界
---

# 单元测试编写技能（unit-test-write）— Python 版
...
你是测试驱动质量的工程师。你的职责分两段：编码前让失败测试先存在，编码后补齐边界、降级、回归测试。
...
```python
def test_trigger_hitl_when_budget_overrun_exceeds_15_percent():
    plan = plan_with_cost(11500)
    result = budget_service.check(plan)
    assert result.requires_human_intervention is True

def test_return_offline_estimate_when_map_api_unavailable(mocker):
    mocker.patch("service.router.MapClient.distance",
                 side_effect=TimeoutError)
    result = route_service.plan(req)
